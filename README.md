...
`excerpt`는 한국어뿐 아니라 영어·스페인어 발췌문도 그대로 붙여넣으면 돼요 — 언어를 자동으로 인식해서, 2번 슬라이드에 원문 한 문장 + 한국어 번역을 넣어줘요 (자세한 규칙은 `CLAUDE.md`의 "외국어 뉴스 지원" 참고).

쓰레드 문구는 같은 방식으로 **"Threads Post"** 워크플로우를 실행하면 돼요 (`topic` 칸에 주제 입력).

처음 한 번만 설정이 필요해요 — 아래 [처음 설치하는 법](#처음-설치하는-법) 참고. 매주 실제로 쓸 때 화면 그대로 따라 하는 쉬운 버전은 [USAGE.md](USAGE.md)를 참고하세요.
